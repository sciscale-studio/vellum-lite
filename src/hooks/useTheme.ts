...
import { isDarkTheme, isThemeAvailable, isThemeName } from '../theme';
  ...
  useEffect(() => {
    async function initStore() {
      try {
        ...
        if (savedTheme && isThemeName(savedTheme.value) && isThemeAvailable(savedTheme.value)) {
          setTheme(savedTheme.value);
        } else if (savedTheme && isThemeName(savedTheme.value)) {
          setTheme('dark');
        }
