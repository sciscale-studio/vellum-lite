...
function App() {
  ...
  // Ctrl + mouse wheel = adjust font size. Accumulates deltaY across events
  // so trackpad pinch-zoom (many small deltas) doesn't blow past the entire
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (Math.abs(accumDelta) >= 50) {
        const dir = accumDelta > 0 ? -1 : 1;
        const state = useAppStore.getState();
        state.setFontSize(Math.max(12, Math.min(28, state.fontSize + dir)));
        accumDelta = 0;
      }
    };
